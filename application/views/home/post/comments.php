...
<?php $sorted_order_ids = $this->post_model->get_comments($post->comment_id);
   if ($sorted_order_ids == NULL) { ; }
   else {
      foreach ($sorted_order_ids as $order_id) {
         $comment = $this->post_model->get_comment_info($post->comment_id, $order_id);
      ...
      <!-- IF NOT SUBCOMMENTS -->
               <?php if ($order_id == end($sorted_order_ids)) {?><div class="row"><div class="col s12 m10 offset-m2"><?php include 'application/views/petridish/post/subcomments.php'; ?></div></div><?php } ?>
      <?php }
            if ($this->post_model->comment_type($comment->order_id) == "subcomment") { ?>
               <div class="row">
                  ...
                  <?php if ($this->post_model->same_subcomment($comment->comment_id, $order_id ))
                  { ?>
                  <div class="col s12 m10 offset-m2"><?php include 'application/views/petridish/post/subcomments.php'; ?></div>
